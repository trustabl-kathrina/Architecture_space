...
2. ``.env`` files (monorepo root, then ``services/api/.env``).
...
# services/api/src/kew_api/config/settings.py → repo root is parents[5]
_CONFIG_DIR = Path(__file__).resolve().parent
_PACKAGE_ROOT = _CONFIG_DIR.parent
_SRC_ROOT = _PACKAGE_ROOT.parent
_API_ROOT = _SRC_ROOT.parent
_SERVICES_ROOT = _API_ROOT.parent
_REPO_ROOT = _SERVICES_ROOT.parent
