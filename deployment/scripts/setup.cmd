...
call "%~dp0_root.cmd"
set "VENV=%ROOT%\backend\doc-factory\.venv"
...
if errorlevel 1 (
  ...
  echo Close terminals running the API, then rerun deployment\scripts\setup.cmd
  exit /b 1
)
if not "%PIP_RC%"=="0" (
...
pushd "%ROOT%\backend\doc-factory"
"%PIP%" install -e .
if errorlevel 1 (
...
if "%NODE_OK%"=="1" (
  ...
  where npm >nul 2>&1
  if not errorlevel 1 (
    echo pnpm not found - using npm in frontend...
    pushd "%ROOT%\frontend"
    call npm install
    if errorlevel 1 (
      popd
    ...
    echo ERROR: Node.js not found. Install from https://nodejs.org/ then rerun deployment\scripts\setup.cmd
    exit /b 1
  )
)
...
echo.
echo Setup complete.
echo   API:  deployment\scripts\dev-api.cmd
echo   UI:   deployment\scripts\dev-web.cmd
